 ...
 * Endpoints used:
 *   soccernew/home?json=1   — live fixtures and scores for football leagues
 */
...
export async function findMatchStaticId(): Promise<string> {
  ...
  try {
    ...
    console.warn(
      `[goalserve] ${home} vs ${away} not found in soccernew/home feed`,
    );
  } catch (err) {
...
export async function pollLiveMatch(
  ...
  try {
    ...
    const feedPaths = ["soccernew/home?json=1", "soccernew/d1?json=1"] as const;
