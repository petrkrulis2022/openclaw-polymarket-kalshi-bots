/**
 * tennis-bot.ts — Live Score-Triggered Trading Bot (Tennis)
 *
 * Strategy (manual score-trigger arbitrage):
 *   1. User confirms set win in dashboard before Gamma/CLOB reprices
 ...
 *
 * Tokens traded (Home player binary YES/NO):
 *   Home wins set → buy Home Player YES, sell after repricing up
 *   Away wins set → buy Home Player NO  (away wins match), sell after repricing up
 *
 * Run:
 *   cd bots/tennis && npx tsx scripts/tennis-bot.ts
...
// Last CLOB prices seen during the live poll. Asks are the pre-set reference
...
async function fetchTradeAmountFromOrchestrator(): Promise<void> {
  ...
  try {
    const res = await fetch(
      `${config.orchestrator.baseUrl}/users/${userAddress}/bots/tennis-bot/trade-amount`,
      { signal: AbortSignal.timeout(5_000) },
    );
    if (res.ok) {
...
async function loadWatchedGamesFromOrchestrator(): Promise<void> {
  ...
  try {
    const res = await fetch(
      `${config.orchestrator.baseUrl}/users/${userAddress}/bots/tennis-bot/watched-games`,
      { signal: AbortSignal.timeout(5_000) },
    );
    if (!res.ok) {
      console.warn(
    ...
    if (activeMatchSlug && nextBindingKey !== activeMarketBindingKey) {
      try {
        ...
        console.log(
          `[watch] Using watched match market slug=${activeMatchSlug} (${activeTeamHome} vs ${activeTeamAway})`,
        );
      } catch (err) {
        console.warn(
    ...
    if (watchedStaticId) {
      ...
      if (
        ...
        console.log(
          `[watch] Using watched match staticId=${staticId} (${activeTeamHome} vs ${activeTeamAway})`,
        );
      }
    }
    ...
    if (
      ...
      lastScoreHome = NaN;
      lastScoreAway = NaN;
      // Drop the previous match's cached prices.
      lastYesAsk = 0;
      lastNoAsk = 0;
      lastYesBid = 0;
      lastNoBid = 0;
...
async function ensureMarketReady(): Promise<void> {
  while (true) {
    if (!activeMatchSlug) {
      ...
      console.warn(
        "[setup] No match slug configured yet; waiting for watched match selection...",
      );
    } else {
      try {
        console.log(
...
> {
  ...
  const isHomeScore = scorer === "home";
  const tokenId = isHomeScore ? market.yesTokenId : market.noTokenId;
  const label = isHomeScore ? `${activeTeamHome} WIN` : `${activeTeamAway} WIN`;
  const scoringPlayer = scorer === "home" ? state.teamHome : state.teamAway;

  console.log(
    `\n[trade] 🚨 SET: ${scoringPlayer} won a set. Score: ${state.scoreHome}-${state.scoreAway} (set ${state.scoreHome + state.scoreAway})`,
  );
  console.log(
  ...
  for (let attempt = 1; attempt <= MAX_BUY_ATTEMPTS; attempt++) {
    try {
      ...
      console.log(
        `[trade] BUY attempt ${attempt}/${MAX_BUY_ATTEMPTS}: ${askSummary} worst=${fmt(buyWorstPriceCap, 6)} spend=${fmt(spendAmountUsd, 6)}`,
      );
...
async function checkAndSell(
  ...
  try {
    const sellWorstPrice = Math.max(0.01, bestBid - (market.orderPriceMinTickSize ?? 0.01) - 0.01);
...
function printReport(): void {
  console.log("\n" + "═".repeat(60));
  console.log("TENNIS BOT SESSION REPORT");
...
async function lifecycleMonitorLoop(): Promise<void> {
  while (!gameIsOver) {
    ...
    // + the pre-set BUY slippage cap). Quiet: no per-second log spam during the game.
    ...
    if (lifecycle) {
      if (lifecycleLooksEnded(lifecycle)) {
        ...
        console.log(
          `[pm] ⏱️  Market lifecycle indicates match over (status=${lifecycle.gamma.rawStatus}, closed=${lifecycle.gamma.closed}, resolved=${lifecycle.gamma.resolved})`,
        );
        gameIsOver = true;
        break;
      }
...
async function sellMonitorLoop(): Promise<void> {
  ...
  if (openPosition) {
    console.log("[sell]  Match over — force-closing open position");
    ...
    if (openPosition) {
      console.warn("[sell]  Match over: force-close failed after 10 attempts — position requires manual action via /force-sell");
    }
  }
}

async function waitForMatchStart(): Promise<void> {
  ...
  console.log(`\n[bot]  Waiting for match start (match=${matchHint})`);
  console.log(
    `[bot]  Polling Polymarket lifecycle every ${config.preGamePollMs / 1000}s for match start...\n`,
  );
...
httpApp.get("/watchlist-state/:key", (req, res) => {
  ...
  if (!row) {
    res.status(404).json({ error: "Watchlist match not found" });
    return;
  }
  res.json({
    ok: true,
...
httpApp.post("/manual-trigger", async (req, res) => {
  ...
  if (!key) {
    return res.status(400).json({
      ok: false,
      reason: "invalid_request",
      error: "No watched match selected",
    });
  }
  ...
  const triggerResult = await onScoreEvent(sideRaw, manualState, {
    source: "manual",
    clientTriggeredAtMs,
    serverReceivedAtMs,
  });
...
httpApp.listen(config.port, () => {
  console.log(`[api]  Tennis Bot HTTP API listening on :${config.port}`);
});
...
async function main(): Promise<void> {
  console.log("═".repeat(60));
  console.log("TENNIS BOT — Live Score Arbitrage");
  console.log("═".repeat(60) + "\n");
  ...
  while (true) {
    ...
    console.log("═".repeat(60));
    console.log(
      `TENNIS BOT — ${activeTeamHome} vs ${activeTeamAway} | Live Score Arbitrage`,
    );
    console.log(
      `match=${activeMatchSlug || "(awaiting watched match slug)"} | budget=${runtimeMaxPositionUsd} USDC`,
    );
    console.log("═".repeat(60) + "\n");
    ...
    if (gameIsOver) {
      printReport();
      console.log("[bot]  Match already over — waiting for next match...\n");
      await sleep(30_000);
      continue;
    }
    ...
    printReport();
    console.log("[bot]  Match over — resetting for next watched match...\n");
    await sleep(15_000);
  }
}
