 ...
 * POST /recall
 *
 * Transfers USD₮ from a bot wallet (index 1–3) back to the treasury wallet (index 0).
 *
 * Body: { "botId": 1 | 2 | 3, "amountUsdT": "5.0" }
 *
 * Validations:
 *   - botId must be 1, 2, or 3
 *   - amountUsdT must be a positive decimal string
 *   - Bot wallet must have sufficient USD₮ balance
 *   - Bot wallet must have POL (native token) for gas
 *
 * Gas note: the BOT wallet pays the EVM transaction fee in POL.
 *           Fund bot wallets from https://faucet.polygon.technology (Amoy testnet).
 */
...
router.post("/", async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    // Derive both accounts in parallel; we need the treasury address as recipient
    ...
    const [treasuryAddress, botAddress, balanceRaw, nativeBalanceRaw] =
      await Promise.all([
        ...
        botAccount.getTokenBalance(USDT_TOKEN_ADDRESS), // ← bot's USD₮
        botAccount.getBalance(), // ← bot's POL for gas
      ]);

    // We only needed the treasury account for its address; dispose it now
    try {
    ...
    if (balance < amount) {
      return res.status(400).json({
        ...
        message: `Bot ${botId} wallet has ${formatUsdT(balance)} USD₮ but ${formatUsdT(amount)} was requested.`,
      });
    }
    ...
    if (nativeBalance === 0n) {
      return res.status(400).json({
        error: "No gas",
        message:
          `Bot ${botId} wallet has no POL to pay the transaction fee. ` +
    ...
    const result = await botAccount.transfer({
